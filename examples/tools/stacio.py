...
def remote_asset(href: str, media: str, roles: list[str], title: str,
    ...
    `file:checksum`. `PORTO-CORE-028` makes both fields SHOULD, so omitting one is
    conformant rather than a gap to baseline, and `PORTO-CORE-030` requires any
    value an asset does carry to match the bytes its `href` resolves to. A mirror
    that never reads 1.86 TB of upstream COGs cannot satisfy that for a digest, so
    publishing one would break a MUST. rashid reports the absence as `PTL-AST-003`
    at warning severity, which is the accurate outcome, not a suppressed error.
    ...
    would be this catalog asserting something about bytes it never read.
    """
...
def quickstart_block(kind: str, data_name: str) -> list[str]:
    ...
    if kind == "raster-mosaic":
        return [
            ...
            "import rasterio",
            "",
            "# href is an image asset href read from the index above, which points",
            "# at the upstream host, https://naipeuwest.blob.core.windows.net/ .",
...
def provenance_block(spec: dict, providers: list[dict], facts: dict) -> list[str]:
    ...
    if kind == "vector":
        ...
        lines.append(f"Scenes, {facts['n']:,}. Cloud-native asset, "
                     f"{facts['data_name']} (stac-geoparquet item mirror).")
        lines.append("Scene bytes are hosted upstream and referenced by URL, not "
                     "copied here. Each scene asset carries file:size read from the "
                     "host and no file:checksum, which core.md asks for as a SHOULD "
                     "and which this catalog cannot compute for bytes it never reads.")
    else:
...
def build_collection(spec: dict, host: dict, out_root: Path, cache: Path,
    ...
    if kind in ("vector",):
        ...
        if deriv.get("thumbnail", True):
            ...
            assets["thumbnail"] = asset(th, MEDIA["png"], ["thumbnail"], _thumb_desc(thumb))
        # Scene COGs are item-level, so the Collection describes no bands and
        # carries no CRS of its own, and the mirror's schema is stac-geoparquet's
        # rather than an authored one, so there is no manifest column prose.
        cols, bands, code = [], [], None
...
def _facts_from_collection(spec: dict, coll: dict) -> dict:
    ...
    A raster-mosaic Collection has no `data` asset. core.md scopes `data` to the
    primary cloud-native file and this kind hosts none, its scene COGs are
    item-level, so the collection-level artifact is the `items` mirror. Reading
    `assets["data"]` unconditionally raised KeyError on that kind."""
    assets = coll["assets"]
    data = assets.get("data") or assets["items"]
    data_name = data["href"].removeprefix("./")
    cols = describe_columns(data.get("table:columns", []), spec.get("columns") or {})
    join = dict(spec.get("join") or {})
    if join:
        join["this_file"] = data_name
    # The mirror asset carries no `table:row_count`, so a mosaic's scene count
    # comes from its own rel="item" links, which build_collection wrote one per
    # scene. Without this the docs-only path regenerates "Scenes, 0".
    n = (sum(1 for lk in coll.get("links", []) if lk.get("rel") == "item")
         if spec["kind"] == "raster-mosaic" else data.get("table:row_count", 0))
    return {
...
def regen_docs(manifest: dict, out: Path, cache: Path) -> None:
    ...
    for spec in manifest["collections"]:
        ...
        # raster-mosaic carries no `data` asset, see _facts_from_collection.
        data = coll["assets"].get("data") or coll["assets"]["items"]
